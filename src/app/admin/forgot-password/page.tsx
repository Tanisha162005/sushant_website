...
import { Shield, Mail, Loader2, ArrowLeft, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function AdminForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(data.message || 'If an admin account exists with this email, a password reset link has been sent.');
    } catch {
      setStatus('error');
          ...
          {status !== 'success' && (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  if (status !== 'loading') e.currentTarget.style.boxShadow = '0 6px 30px rgba(168, 85, 247, 0.5)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = '0 4px 20px rgba(168, 85, 247, 0.3)';
                }}
              >
                {status === 'loading' ? <Loader2 style={{ width: 20, height: 20, animation: 'spin 1s linear infinite' }} /> : 'Send Reset Link'}
              </button>
            </form>
          )}
